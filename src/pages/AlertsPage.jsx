...
export default function AlertsPage() {
  ...
  const allAlerts = [...mlAlerts, ...fleetAlerts].sort((a) => a.severity === 'critical' ? -1 : 1);
  const filtered = filter === 'all' ? allAlerts
    : filter === 'stock' ? allAlerts.filter((a) => a.type !== 'fleet')
    : allAlerts.filter((a) => a.type === 'fleet');
    ...
    <div className="p-6 space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-surface-500 mt-0.5">Stock warnings, fleet delays & ML recommendations</p>
        </div>
        <div className="flex gap-2">
          {['all', 'stock', 'fleet'].map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === 'all' ? `All (${allAlerts.length})` : f === 'stock' ? 'Stock' : 'Fleet'}
            </button>
          ))}
        </div>
      </div>
        ...
        {filtered.map((alert) => (
          <div key={alert.id}
            ...
            <div className={`mt-1 w-2.5 h-2.5 rounded-full flex-shrink-0 ${
              alert.severity === 'critical' ? 'bg-danger-500 animate-pulse' : 'bg-warning-500'
            }`} />
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                <span className="text-sm font-semibold text-surface-900">{alert.title}</span>
                <span className={tagStyle[alert.tag] || 'badge-neutral'}>{alert.tag}</span>
                {alert.timestamp && (
